...
parser.add_argument("--profile", choices=("bringup", "s9u-control", "s9u-nobti"), default="bringup")
...
if args.profile in ("s9u-control", "s9u-nobti"):
...
if args.profile == "s9u-nobti" and "CONFIG_ARM64_BTI_KERNEL=y" in config:
    raise SystemExit("s9u-nobti control must keep CONFIG_ARM64_BTI_KERNEL disabled")
if args.profile == "s9u-nobti" and "# CONFIG_ARM64_BTI_KERNEL is not set" not in config:
    raise SystemExit("s9u-nobti control did not resolve ARM64_BTI_KERNEL to n")
