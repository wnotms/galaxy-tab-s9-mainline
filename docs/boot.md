...
构建脚本只生成离线镜像；`scripts/twrp-boot-test.py` 默认只读预检，指定 `--flash` 才写入四个启动分区，指定 `--restore` 才恢复备份。**首次实机日志确认 ABL 交接主线内核，但约 31 毫秒后发生固件报告的 NoC 致命错误；第二次同名保留区节点方案出现 ABL 添加／保留失败，未取得本次主线输出。两次均未进入 USB 诊断环境，原四个启动分区已恢复并回读校验**。见 [首次启动记录](boot-test-20260918.md)及 [第二次启动记录](boot-test-20260918-second.md)。结构和哈希检查不能替代实机启动验证。

## 镜像组合

已完成 [第三次实机测试](boot-test-20260918-third.md)，移除预建的三个 ABL 保留区节点后不再出现第二次的 ABL 添加／保留错误，但未取得本次主线输出及 USB 枚举。日志已回收，原四个启动分区已恢复，设备留在 TWRP。

随后已完成 [第四次实机测试](boot-test-20260918-fourth.md)，将持久 console 注册提前到 `console_initcall`，但未取得新增到达标记、主线输出或 USB 枚举，停止位置仍未确认。日志已归档，原四个启动分区已恢复并回读校验，设备留在 TWRP。

已完成 [第五次实机测试](boot-test-20260918-fifth.md)，在日志正文及头部写入后增加同步 PoC 缓存清理，但仍未取得 console 标记、主线输出或 USB 枚举，停止位置未确认。日志已归档，原四个启动分区已恢复并回读校验，设备留在 TWRP。

已完成 [第六次实机测试](boot-test-20260918-sixth.md)，新增五个 setup_arch 早期检查点，但持久日志均未取得；仍没有主线输出和 USB 枚举，停止位置未确认。日志已归档，原四个启动分区已恢复并回读校验，设备留在 TWRP。
