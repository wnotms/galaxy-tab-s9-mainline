...
构建脚本只生成离线镜像；`scripts/twrp-boot-test.py` 默认只读预检，指定 `--flash` 才写入四个启动分区，指定 `--restore` 才恢复备份。**首次实机写入与回读已通过，但重启后未观察到 USB，主线启动尚未确认**；见 [首次启动记录](boot-test-20260918.md)。结构和哈希检查不能证明 ABL 接受设备树、内核正常运行或 USB 正常枚举。
...
如果不能枚举 USB，按设备的恢复组合回到 TWRP，通过 ADB 读取 `/proc/last_kmsg`。日志驱动与 TWRP 共用 `sec_log_buf`；能否跨此次重启保留日志需要实测，不要将没有日志直接等同于内核未执行。
