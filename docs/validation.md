...
第五版新增 `clean-sec-log-cache-before-reset.patch`，先清理日志正文到 PoC，再发布 index / previous_index 并清理头部，保留 WB 映射和注册时机。全部六个补丁在干净固定源码上应用通过，与实际编译源码逐字节一致；`make bundle`、四镜像检查和 `make check` 通过，目标文件包含正文首段、回绕第二段及头部的三处缓存清理调用重定位。[第五次实机测试](boot-test-20260918-fifth.md)未枚举 USB，持久日志仍没有 console 标记、主线 printk 或 initcall 跟踪；缓存清理路径是否执行、停止位置和复位保留行为均未确认。日志已归档，原四个启动分区已恢复并回读校验。
...
按用户 test11.md 构建 GPIO 36–39 保留候选，仅改变 Test10 基线 TLMM 的 gpio-reserved-ranges。原内核／配置和 initramfs 复用，最终 vendor_boot DTB 已核对，四镜像检查、全树单属性差异及篡改拒绝检查通过。用户随后授权冷启动测试，四分区写入／回读通过，但自动进入 TWRP；ABL 记录 PARAM_BOOT_RECOVERY_ENTER 并选择 recovery。优先保存的 pstore 为空，last_kmsg 无新主线日志，结果 INCONCLUSIVE，NoC 状态未知。原四分区已恢复且六分区哈希匹配，设备留在 TWRP。用户随后授权同镜像热启动复测，重新刷入和回读通过，正常重启已请求，用户观察停在三星标识，无目标 USB／新网络接口／ADB；等待返回 TWRP 采集日志，本轮尚未恢复原分区。见 [GPIO Test11](boot-test-20260918-eleventh.md)。
